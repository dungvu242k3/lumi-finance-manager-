import { AlertCircle, Download, Edit2, FileText, Plus, Search, Trash2, Upload, X } from 'lucide-react';
...
export const Cost: React.FC<Props> = ({ transactions, setTransactions, accounts, lockedKeys }) => {
  ...
  const costAccounts = accounts.filter(a => a.type === TransactionType.EXPENSE && a.status === 'Active');
    ...
    const headers = [
      {
        ...
        Nguon_Chi: 'Chi từ tài khoản công ty',
        Chi_Nhanh: 'Hà Nội',
        Thi_Truong: 'US',
        Ma_TK: '2.1US',
        Noi_Dung: 'Ví dụ chi phí',
        So_Tien: 500000,
        Hinh_Thuc: 'CK từ TK Công ty'
      }
    ];

    const ws = XLSX.utils.json_to_sheet(headers);
    XLSX.utils.sheet_add_aoa(ws, [['Ngay', 'Nguon_Chi', 'Chi_Nhanh', 'Thi_Truong', 'Ma_TK', 'Noi_Dung', 'So_Tien', 'Hinh_Thuc']], { origin: 'A1' });

    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, "Mau_Chi_Phi");
    XLSX.writeFile(wb, "mau_chi_phi.xlsx");
  };
      ...
      reader.onload = (e) => {
        try {
          ...
          if (importedTransactions.length < jsonData.length) {
            ...
            alert(`Đã import thành công ${importedTransactions.length} khoản chi. Đã bỏ qua ${blockedCount} dòng do dữ liệu lỗi hoặc thuộc Tài khoản/Chi nhánh đã KHÓA SỔ.`);
          } else {
            alert(`Đã import thành công ${importedTransactions.length} khoản chi!`);
          }
  ...
  const filteredTrans = costTransactions.filter(t =>
    ...
    normalizeString(t.source).includes(normalizeString(searchTerm))
  );
    ...
    <div className="space-y-6">
      <div className="flex flex-col xl:flex-row justify-between items-start xl:items-center gap-4 bg-white p-4 rounded-lg shadow-sm border border-slate-200">
        ...
        <div className="flex flex-wrap items-center gap-2 w-full xl:w-auto justify-end">
          ...
          <button
            onClick={() => {
              const exportData = costTransactions.map((t, index) => ({
                'STT': index + 1,
                'Ngày': t.date,
                'Nguồn chi': t.source,
                'Chi nhánh': t.branch,
              ...
              const wb = XLSX.utils.book_new();
              XLSX.utils.book_append_sheet(wb, ws, "Chi_Phi");
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-lg overflow-hidden animate-in fade-in zoom-in duration-200">
            <div className="bg-slate-50 px-6 py-4 border-b border-slate-200 flex justify-between items-center">
              <h3 className="font-bold text-lg text-slate-800">Hướng dẫn Import Chi Phí Excel</h3>
            ...
            <div className="p-6 space-y-4">
              <div className="bg-red-50 border border-red-200 rounded-lg p-4 text-sm text-red-800">
                ...
                <ul className="list-disc list-inside space-y-1 ml-2">
                  <li><strong>Ngay:</strong> Ngày chi (YYYY-MM-DD)</li>
                  <li><strong>Nguon_Chi:</strong> Nguồn tiền (VD: TK Công ty)</li>
                  <li><strong>Chi_Nhanh:</strong> {Object.values(Branch).join(', ')}</li>
                  <li><strong>Thi_Truong:</strong> {Object.values(Market).join(', ')}</li>
                  <li><strong>Ma_TK:</strong> Mã trong Master Data (VD: 2.1US)</li>
              ...
              <div className="text-sm text-slate-500 italic">
                * Vui lòng tải file mẫu và nhập liệu đúng cột.
              </div>
            </div>
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-2xl overflow-hidden animate-in fade-in zoom-in duration-200">
            <div className="bg-red-600 px-6 py-4 border-b border-red-700 flex justify-between items-center text-white">
              <h3 className="font-bold text-lg flex items-center gap-2"><AlertCircle size={20} /> {editingId ? 'Cập nhật Khoản Chi' : 'Ghi nhận Chi Phí'}</h3>
              <button onClick={() => {
                setIsModalOpen(false);
                resetForm();
              }} className="text-red-100 hover:text-white transition-colors"><X size={20} /></button>
            </div>
            <div className="p-6 space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  >
                    <option value="">-- Chọn Nguồn Chi --</option>
                    {costAccounts.map(acc => (
